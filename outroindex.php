<<<<<<< HEAD
<?php

echo "Bom dia ";


?>
=======
<?php
echo "Hello word! criei um repositorio no git";
echo " Para transformar uma pasta em repositório no git use comando git init";
echo "para enviaros arquivos do diretorio para o git "
echo " Crie um arquivo , Para enviar um arquivo para a area de transição use o comando add arquivo ou add --all ";
echo " para enviar para o git  git commit -n de um espaço abre aspas duplas digite um texto dentro das aspas ";
echo "aperte enter  e sera feito um commit no diretorio local";
echo "para subir para o servidor remoto do Github navegue até a pasta e  use o codigo para determinar o nome e email padrão um apos o outro entre aspas e digite enter  
    git config --global user.name "seu nome"
    git config --global user.email "seu email" ";
?>
>>>>>>> 1dcc071fcdec7c9dd622eacbd4b222ac8d6f8302
